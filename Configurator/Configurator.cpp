...
bool Configurator::Load(vector<Record>& content)
{
	...
	if (!inFile.is_open())
	{
		WriteDefault();
		return false;
	}									// If the input file doesn't open, then return
...
bool Configurator::Save(vector<Record>& content)
{
	...
	for (int i=0;i<(int)content.size();i++)									// Loop through each vector
	{
		if (content[i].Comments != "")
			outFile << content[i].Comments<< endl;		
...
vector<string> Configurator::GetSectionNames(string FileName)
{
	vector<string> data;													// Holds the return data
	vector<Record> content;													// Holds the current record													// Holds the current record

	if (Load( content))											// Make sure the file is loaded
	{
		for (int i=0;i<(int)content.size();i++)								// Loop through the content
		{
...
vector<Record> Configurator::GetSectionVector(string SectionName)
{
...
map<string, string> Configurator::GetSectionMap(string SectionName)
{
	...
	if (Load(content))											
	{
		for (int i=0;i<(int)content.size();i++)								
		{
			if((content[i].Section == SectionName) &&						
				(content[i].Key != ""))										
				data[SectionName] = content[i].Value;						
		}
	}
	return data;															
}

bool Configurator::RecordExists(string KeyName, string SectionName)
{
	vector<Record> content;													// Holds the current record													// Holds the current record
...
bool Configurator::SectionExists(string SectionName)
{
	vector<Record> content;													// Holds the current record													// Holds the current record
...
vector<Record> Configurator::GetRecord(string KeyName, string SectionName)
{
	vector<Record> data;													// Holds the return data
	vector<Record> content;													// Holds the current record													// Holds the current record
...
string Configurator::GetValue(string KeyName, string SectionName)
{
...
bool Configurator::SetValue(string KeyName, string Value, string SectionName)
{
	vector<Record> content;													// Holds the current record													// Holds the current record
...
bool Configurator::RenameSection(string OldSectionName, string NewSectionName)
{
	vector<Record> content;													// Holds the current record													// Holds the current record
...
bool Configurator::CommentRecord(CommentChar cc, string KeyName,string SectionName)
{
	vector<Record> content;													// Holds the current record													// Holds the current record
...
bool Configurator::UnCommentRecord(string KeyName,string SectionName)
{
	vector<Record> content;													// Holds the current record													// Holds the current record
...
bool Configurator::CommentSection(char CommentChar, string SectionName)
{
	vector<Record> content;													// Holds the current record													// Holds the current record
...
bool Configurator::UnCommentSection(string SectionName)
{
	vector<Record> content;													// Holds the current record													// Holds the current record
...
bool Configurator::DeleteRecord(string KeyName, string SectionName)
{
	vector<Record> content;													// Holds the current record													// Holds the current record
...
bool Configurator::DeleteSection(string SectionName)
{
	vector<Record> content;													// Holds the current record													// Holds the current record
...
bool Configurator::SetSectionComments(string Comments, string SectionName)
{
	vector<Record> content;													// Holds the current record													// Holds the current record
...
bool Configurator::SetRecordComments(string Comments, string KeyName, string SectionName)
{
	vector<Record> content;													// Holds the current record													// Holds the current record
...
vector<Record> Configurator::GetSections(string FileName)
{
	vector<Record> data;													// Holds the return data
	vector<Record> content;													// Holds the current record													// Holds the current record
...
bool Configurator::Sort(string FileName, bool Descending)
{
	...
	if(!sections.empty())													// Is there anything to process?
	{
		...
		{																		
			...
			vector<Record> records = GetSectionVector(iter->Section ); // Get a list of Records for this section
...
bool Configurator::AddSection(string SectionName)
{
	vector<Record> content;													// Holds the current record													// Holds the current record
...
bool Configurator::Create(string FileName)
{
...
bool Configurator::WriteDefault()
{
	...
	audioConfig.ToVector(vect);
	videoConfig.ToVector(vect);
	Save(vect);
	return true;
}
